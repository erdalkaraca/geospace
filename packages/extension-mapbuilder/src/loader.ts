import { html, render } from "@eclipse-docks/core/externals/lit";
...
function shouldSkipDirectory(dirName: string): boolean {
    ...
    return SKIP_FOLDERS.some(
        (skipFolder) => lowerName === skipFolder.toLowerCase()
    );
}
...
async function collectFilesByExtension(
    ...
    for (const child of children) {
        ...
        if (
            ...
            child.getName().toLowerCase().endsWith(extension)
        ) {
            files.push(childPath);
        } else if (child instanceof Directory) {
            if (!shouldSkipDirectory(child.getName())) {
                const subFiles = await collectFilesByExtension(
                    child,
                    extension,
                    progressMonitor,
                    processedCount
...
export default ({ commandRegistry }: { commandRegistry: CommandRegistry }) => {
    commandRegistry.registerAll({
        command: {
            ...
            name: "Build map",
            description:
                "Builds a publishable map from a .geospace file",
            parameters: [
                {
                    name: "file",
                    description:
                        "Path to the .geospace file relative to workspace root. If not provided, uses the active map editor or selected file.",
                    required: false,
                },
                {
                    name: "env",
                    description:
                        "Path to the environment file relative to workspace root (e.g., 'dev.env', 'prod.env'). Defaults to '.env' if not specified.",
                    required: false,
                },
                {
                    name: "ask",
                    description:
                        "If true, prompts for missing files (map file or env file) when not provided.",
                    required: false,
                },
            ],
        },
        handler: {
            execute: async (context: any) => {
                ...
                if (filePath) {
                    ...
                    if (
                        !resource
                            .getName()
                            .toLowerCase()
                            .endsWith(GEOSPACE_EXTENSION)
                    ) {
                        toastError(
                            `File is not a .geospace file: ${filePath}`
                        );
                        return;
                    }
                    mapFile = resource;
                ...
                if (ask && (!mapFile || !envPath)) {
                    ...
                    await taskService.runAsync(
                        ...
                        async (progressMonitor) => {
                            progressMonitor.message =
                                "Scanning workspace for .geospace files...";
                            const geospaceCount = { value: 0 };
                            geospaceFiles = await collectFilesByExtension(
                                workspace,
                                GEOSPACE_EXTENSION,
                                progressMonitor,
                                geospaceCount
                            ...
                            envFiles = await collectFilesByExtension(
                                workspace,
                                ENV_EXTENSION,
                                progressMonitor,
                                envCount
                    ...
                    const currentGeospacePath =
                        mapFile?.getWorkspacePath();
                    ...
                    if (result.geospaceFile && !mapFile) {
                        ...
                        const resource =
                            await workspace.getResource(filePath);
                        if (!resource || !(resource instanceof File)) {
                            toastError(`File not found: ${filePath}`);
                            return;
                        }
                    ...
                    toastError(
                        "No map editor active, no .geospace file selected, and no file path provided"
                    );
                    return;
                }
                ...
                return browserMapBuilder.buildMapFile(
                    mapFile,
                    envPath
                );
            },
        ...
        contribution: {
            ...
            disabled: () => {
                return !(
                    activePartSignal.get() instanceof GsMapEditor
                );
            },
        } as any,
    });
