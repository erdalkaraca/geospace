...
export function renderStylePreview(style: GsStyle) {
    ...
    if (style.image?.type === 'circle') {
        return renderCirclePreview(style, strokeColor, fillColor, strokeWidth);
    }

    if (style.image?.type === 'regular-shape') {
        return renderRegularShapePreview(style, strokeColor, fillColor, strokeWidth);
    }
    ...
    // Default: show a rectangle with stroke and fill for polygons/lines
    return html`
...
function renderCirclePreview(style: GsStyle, defaultStrokeColor: string, defaultFillColor: string, defaultStrokeWidth: number) {
    ...
    const imgStrokeWidth = Math.min(style.image?.stroke?.width || defaultStrokeWidth, 3);
...
function renderRegularShapePreview(style: GsStyle, defaultStrokeColor: string, defaultFillColor: string, defaultStrokeWidth: number) {
    ...
    const imgStrokeWidth = Math.min(style.image?.stroke?.width || defaultStrokeWidth, 3);
    const opacity = style.image?.opacity ?? 1;
    const rotation = ((style.image?.rotation || 0) + (style.image?.angle || 0)) * (180 / Math.PI);
    
    // Generate star/polygon points
    const pathPoints = generateRegularShapePoints(20, 20, radius, points, style.image?.radius2);
...
function renderIconPreview(style: GsStyle) {
    ...
    if (!src) {
        // No icon source, show a placeholder
        return html`
            <svg width="40" height="40" viewBox="0 0 40 40">
                <rect x="8" y="8" width="24" height="24" fill="none" stroke="${color}" stroke-width="2" stroke-dasharray="4 2" opacity="${opacity}" />
                <text x="20" y="24" text-anchor="middle" font-size="12" fill="${color}" opacity="${opacity}">?</text>
            </svg>
        `;
    }

    // For data URI SVGs with currentColor, we can show them inline
    ...
    // For external URLs, show the image
    return html`
        <div style="
            width: 40px; 
            height: 40px; 
...
function generateRegularShapePoints(cx: number, cy: number, radius: number, numPoints: number, innerRadius?: number): string {
    const points: string[] = [];
