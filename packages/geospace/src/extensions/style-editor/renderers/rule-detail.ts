...
function renderConditionSection(rule: GsStyleRule) {
    return html`
        <div class="form-section">
            <h4>Condition</h4>
            ${rule.condition.geometryType ? html`
                <wa-input 
                    label="Geometry Type" 
                    value="${Array.isArray(rule.condition.geometryType) 
                        ? rule.condition.geometryType.join(', ') 
                        : rule.condition.geometryType}"
                    readonly
                ></wa-input>
            ` : ''}
            ${rule.condition.layerName ? html`
                <wa-input 
                    label="Layer Name" 
                ...
                ></wa-input>
            ` : ''}
            ${rule.condition.property ? renderPropertyCondition(rule.condition.property) : ''}
        </div>
    `;
}

function renderPropertyCondition(property: NonNullable<GsStyleRule['condition']['property']>) {
    return html`
        <wa-input 
            label="Property Key" 
            value="${property.key}"
