...
export interface StyleUpdateHandlers {
    ...
    // Image - common
    onImageTypeChange: (value: 'circle' | 'icon' | 'regular-shape') => void;
...
// Icon preset SVG templates - use {{COLOR}} as placeholder for fill color
const ICON_SVG_TEMPLATES: Record<string, string> = {
    'marker': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="{{COLOR}}" d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>',
...
/**
 * Generates a data URI from an SVG template with the specified color.
 */
function generateIconDataUri(templateName: string, color: string = DEFAULT_ICON_COLOR): string {
...
/**
 * Detects which preset template a data URI was generated from (if any).
 */
function detectPresetFromDataUri(dataUri: string): string | null {
    ...
    try {
        ...
        // Check each template by comparing structure (ignoring color)
        for (const [name, template] of Object.entries(ICON_SVG_TEMPLATES)) {
            // Extract the path 'd' attribute which is unique per icon
...
/**
 * Extracts the fill color from an SVG data URI.
 */
...
function getShapePreset(points: number | undefined, radius2: number | undefined): string {
    ...
    for (const [name, preset] of Object.entries(SHAPE_PRESETS)) {
        if (preset.points === points) {
            if (preset.radius2 !== undefined && radius2 !== undefined) {
                return name;
            } else if (preset.radius2 === undefined && radius2 === undefined) {
                return name;
            }
        }
    }
...
export function renderStyleDetail(
    ...
    return html`
        <div class="detail-panel">
            ...
            ${geometryTypes.length > 0 ? html`
                <p class="geometry-types">Geometry types: ${geometryTypes.join(', ')}</p>
            ` : ''}
...
function renderStrokeSection(style: GsStyle, handlers: StyleUpdateHandlers) {
    ...
    return html`
        <div class="form-section">
            ...
            <wa-color-picker 
                ...
                opacity
                size="small"
                @change=${(e: any) => handlers.onStrokeColorChange(e.target.value)}
            ></wa-color-picker>
            <wa-input 
                type="number" 
                label="Width" 
                ...
                min="0"
                step="0.5"
                @input=${(e: any) => handlers.onStrokeWidthChange(parseFloat(e.target.value))}
            ></wa-input>
            <wa-select 
                label="Line Style"
                value="${currentDashPreset}"
                @change=${(e: any) => handlers.onStrokeLineDashChange(LINE_DASH_PRESETS[e.target.value])}
            >
            ...
            <wa-select 
                ...
                @change=${(e: any) => handlers.onStrokeLineCapChange(e.target.value)}
            >
            ...
            <wa-select 
                ...
                @change=${(e: any) => handlers.onStrokeLineJoinChange(e.target.value)}
            >
...
function renderFillSection(style: GsStyle, handlers: StyleUpdateHandlers) {
    return html`
        <div class="form-section">
            <h4>Fill</h4>
            <wa-color-picker 
                ...
                opacity
                size="small"
                @change=${(e: any) => handlers.onFillColorChange(e.target.value)}
            ></wa-color-picker>
        </div>
    `;
}

function renderImageSection(style: GsStyle, handlers: StyleUpdateHandlers) {
    ...
    return html`
        <div class="form-section">
            ...
            <wa-select 
                label="Symbol Type"
                value="${imageType}"
                @change=${(e: any) => handlers.onImageTypeChange(e.target.value)}
            >
            ...
            ${imageType === 'circle' ? renderCircleProperties(style, handlers) : ''}
            ${imageType === 'regular-shape' ? renderRegularShapeProperties(style, handlers) : ''}
            ${imageType === 'icon' ? renderIconProperties(style, handlers) : ''}
            
            ${style.image ? renderCommonImageProperties(style, handlers) : ''}
        </div>
    `;
}

function renderCircleProperties(style: GsStyle, handlers: StyleUpdateHandlers) {
    return html`
        ...
        <wa-input 
            ...
            min="1"
            step="1"
            @input=${(e: any) => handlers.onImageRadiusChange(parseFloat(e.target.value))}
        ></wa-input>
        <wa-color-picker 
            label="Fill Color" 
            ...
            opacity
            size="small"
            @change=${(e: any) => handlers.onImageFillColorChange(e.target.value)}
        ></wa-color-picker>
        <wa-color-picker 
            label="Stroke Color" 
            ...
            opacity
            size="small"
            @change=${(e: any) => handlers.onImageStrokeColorChange(e.target.value)}
        ></wa-color-picker>
        <wa-input 
            type="number" 
            label="Stroke Width" 
            ...
            min="0"
            step="0.5"
            @input=${(e: any) => handlers.onImageStrokeWidthChange(parseFloat(e.target.value))}
        ></wa-input>
    `;
}

function renderRegularShapeProperties(style: GsStyle, handlers: StyleUpdateHandlers) {
    const currentPreset = getShapePreset(style.image?.points, style.image?.radius2);
    ...
    return html`
        ...
        <wa-select 
            ...
            @change=${(e: any) => {
                ...
                if (preset) {
                    ...
                    handlers.onImageRadius2Change(preset.radius2 !== undefined ? preset.radius2 * (style.image?.radius || 10) : undefined);
                }
            }}
        >
        ...
        <wa-input 
            ...
            max="12"
            step="1"
            @input=${(e: any) => handlers.onImagePointsChange(parseInt(e.target.value))}
        ></wa-input>
        <wa-input 
            type="number" 
            label="Outer Radius" 
        ...
        <wa-switch 
            ...
            @change=${(e: any) => {
                if (e.target.checked) {
                    handlers.onImageRadius2Change((style.image?.radius || 10) * 0.4);
                } else {
        ...
        ${isStar ? html`
            <wa-input 
                ...
                min="1"
                step="1"
                @input=${(e: any) => handlers.onImageRadius2Change(parseFloat(e.target.value))}
            ></wa-input>
        ` : ''}
        <wa-input 
            type="number" 
            ...
            step="15"
            @input=${(e: any) => handlers.onImageAngleChange(parseFloat(e.target.value) * Math.PI / 180)}
        ></wa-input>
        <wa-color-picker 
            label="Fill Color" 
            value="${style.image?.fill?.color || '#0064ff'}"
...
/**
 * Converts raw SVG markup to a base64 data URI with proper width/height attributes.
 * Ensures the SVG has explicit dimensions for proper rendering in OpenLayers.
 */
...
function renderIconProperties(style: GsStyle, handlers: StyleUpdateHandlers) {
    let currentSrc = style.image?.src || '';
    const isDataUri = currentSrc.startsWith('data:');
    
    // Detect if current src is from a preset template
    const detectedPreset = isDataUri ? detectPresetFromDataUri(currentSrc) : null;
    let isPreset = detectedPreset !== null;
    const isCustomSvg = isDataUri && !isPreset;
    const isUrl = !isDataUri && currentSrc.length > 0;
    
    // Extract current fill color from the SVG, or use default
    const extractedColor = isDataUri ? extractColorFromDataUri(currentSrc) : null;
    const currentColor = extractedColor || style.image?.color || DEFAULT_ICON_COLOR;
    
    // Auto-generate default marker if no src is set
    // This ensures icons are visible when first switching to icon type
    if (!currentSrc) {
        const defaultDataUri = generateIconDataUri('marker', currentColor);
        // Trigger the src update asynchronously to avoid render issues
    ...
    return html`
        ...
        <wa-select 
            ...
            @change=${(e: any) => {
                ...
                if (ICON_SVG_TEMPLATES[value]) {
                    // Generate data URI with current color
        ...
        ${isUrl || currentMode === 'custom-url' ? html`
            <wa-input 
                label="Icon URL" 
                value="${currentSrc}"
                placeholder="https://example.com/icon.svg"
                @input=${(e: any) => handlers.onImageSrcChange(e.target.value)}
            ></wa-input>
        ` : ''}
        
        ${currentMode === 'paste-svg' ? html`
            <wa-textarea 
        ...
        ${isPreset ? html`
            <wa-color-picker 
                ...
                size="small"
                @change=${(e: any) => {
                    // Regenerate the preset with new color
                    const presetName = detectedPreset || currentMode;
                    if (ICON_SVG_TEMPLATES[presetName]) {
                        const dataUri = generateIconDataUri(presetName, e.target.value);
                        handlers.onImageSrcChange(dataUri);
                    }
                }}
            ></wa-color-picker>
        ` : html`
            <wa-color-picker 
                ...
                opacity
                size="small"
                @change=${(e: any) => handlers.onImageColorChange(e.target.value)}
            ></wa-color-picker>
        ...
        <wa-input 
            ...
            max="10"
            step="0.1"
            @input=${(e: any) => handlers.onImageScaleChange(parseFloat(e.target.value))}
        ></wa-input>
    `;
}

function renderCommonImageProperties(style: GsStyle, handlers: StyleUpdateHandlers) {
    // Skip scale for icons as it's shown in the icon section
    ...
    return html`
        ...
        <wa-input 
            ...
            step="15"
            @input=${(e: any) => handlers.onImageRotationChange(parseFloat(e.target.value) * (Math.PI / 180))}
        ></wa-input>
        <wa-slider 
            label="Opacity"
            ...
            max="100"
            step="5"
            @change=${(e: any) => handlers.onImageOpacityChange(e.target.value / 100)}
        ></wa-slider>
        ${showScale ? html`
            <wa-input 
                type="number" 
