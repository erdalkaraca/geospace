...
export function getGeometryTypesForStyle(styleId: string, rules: GsStyleRule[]): GsGeometryType[] {
...
export function isPointGeometry(geometryTypes: GsGeometryType[]): boolean {
    return geometryTypes.some(type => 
        type === GsGeometryType.Point || type === GsGeometryType.MultiPoint
    );
}

export function isLineGeometry(geometryTypes: GsGeometryType[]): boolean {
    return geometryTypes.some(type => 
        type === GsGeometryType.LineString || type === GsGeometryType.MultiLineString
    );
}

export function isPolygonGeometry(geometryTypes: GsGeometryType[]): boolean {
    return geometryTypes.some(type => 
...
export function shouldShowProperty(property: StylePropertyType, geometryTypes: GsGeometryType[]): boolean {
