...
export class WorkspaceModuleResolver {
    ...
    private async doResolveModule(resolvedPath: string): Promise<string> {
        ...
        try {
                    ...
                    if (match.isDynamic) {
                        ...
                        if (firstQuoteIndex === -1) {
                            ...
                            if (lastQuoteIndex !== -1 && lastQuoteIndex > firstQuoteIndex) {
                                newImport = match.fullMatch.slice(0, firstQuoteIndex + 1) + importBlobUrl + match.fullMatch.slice(lastQuoteIndex);
                            } else {
                                newImport = match.fullMatch;
                            }
                        }
            ...
            this.moduleCache.set(resolvedPath, { code, workspacePath: resolvedPath, blobUrl });
            return blobUrl;
        } finally {
