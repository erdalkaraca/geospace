...
${E}:${u}:${W}: ERROR: ${M}${w.text}`}).join("");let p=new Error(e);for(const[w,h]of[["errors",n],["warnings",s]])Object.defineProperty(p,w,{configurable:!0,enumerable:!0,get:()=>h,set:E=>Object.defineProperty(p,w,{configurable:!0,enumerable:!0,value:E})});return p}function Se(e,n){for(const s of e)s.detail=n.load(s.detail);return e}function at(e,n,s){if(e==null)return null;let o={},p=i(e,o,"file",D),w=i(e,o,"namespace",D),h=i(e,o,"line",xe),E=i(e,o,"column",xe),u=i(e,o,"length",xe),W=i(e,o,"lineText",D),M=i(e,o,"suggestion",D);if(Q(e,o,n),W){const F=W.slice(0,(E&&E>0?E:0)+(u&&u>0?u:0)+(s&&s>0?s:80));!/[\x7F-\uFFFF]/.test(F)&&!/\n/.test(W)&&(W=F)}return{file:p||"",namespace:w||"",line:h||0,column:E||0,length:u||0,lineText:W||"",suggestion:M||""}}function pe(e,n,s,o,p){let w=[],h=0;for(const E of e){let u={},W=i(E,u,"id",D),M=i(E,u,"pluginName",D),F=i(E,u,"text",D),H=i(E,u,"location",Qe),N=i(E,u,"notes",me),V=i(E,u,"detail",Re),z=`in element ${h} of "${n}"`;Q(E,u,z);let O=[];if(N)for(const X of N){let C={},$=i(X,C,"text",D),g=i(X,C,"location",Qe);Q(X,C,z),O.push({text:$||"",location:at(g,z,p)})}w.push({id:W||"",pluginName:M||o,text:F||"",location:at(H,z,p),notes:O,detail:s?s.store(V):-1}),h++}return w}function Ne(e,n){const s=[];for(const o of e){if(typeof o!="string")throw new Error(`${te(n)} must be an array of strings`);s.push(o)}return s}function $t(e,n){const s=Object.create(null);for(const o in e){const p=e[o];if(typeof p!="string")throw new Error(`key ${te(o)} in object ${te(n)} must be a string`);s[o]=p}return s}function Pt({path:e,contents:n,hash:s}){let o=null;return{path:e,contents:n,hash:s,get text(){const p=this.contents;return(o===null||p!==n)&&(n=p,o=ve(p)),o}}}function Ue(e){let n=e.source;return e.flags&&(n=`(?${e.flags})${n}`),n}var Ct="0.25.11",Tt=e=>Ce().build(e),jt=e=>Ce().context(e),Ot=(e,n)=>Ce().transform(e,n),Dt=(e,n)=>Ce().formatMessages(e,n),Rt=(e,n)=>Ce().analyzeMetafile(e,n),At=()=>{throw new Error('The "buildSync" API only works in node')},Ft=()=>{throw new Error('The "transformSync" API only works in node')},It=()=>{throw new Error('The "formatMessagesSync" API only works in node')},Nt=()=>{throw new Error('The "analyzeMetafileSync" API only works in node')},Ut=()=>(We&&We(),Promise.resolve()),_e,We,Me,Ce=()=>{if(Me)return Me;throw _e?new Error('You need to wait for the promise returned from "initialize" to be resolved before calling this'):new Error('You need to call "initialize" before calling this')},Wt=e=>{e=bt(e||{});let n=e.wasmURL,s=e.wasmModule,o=e.worker!==!1;if(!n&&!s)throw new Error('Must provide either the "wasmURL" option or the "wasmModule" option');if(_e)throw new Error('Cannot call "initialize" more than once');return _e=Mt(n||"",s,o),_e.catch(()=>{_e=void 0}),_e},Mt=(e,n,s)=>re(null,null,function*(){let o,p;const w=new Promise(F=>p=F);if(s){let F=new Blob([`onmessage=((postMessage) => {
...
`,vn=J=>{const P=[],x=[];return[...J.gsMap.controls||[],...J.gsMap.overlays||[]].forEach((j,A)=>{const I=j.src,q=`map_mod${A}`;P.push(`import ${q} from '${I}'`),x.push(`"${I}": ${q}`)}),`
...
`};function dt(J,P){if(J.startsWith("/"))return J.slice(1);const x=P.split("/").filter(A=>A),S=J.split("/").filter(A=>A);if(J.startsWith("./")||J.startsWith("../")||!J.startsWith("/")){const A=x.slice(0,-1);x.length=0,x.push(...A)}for(const A of S)A!=="."&&(A===".."?x.length>0&&x.pop():x.push(A));return x.join("/")}let bn={name:"workspace",setup(J){J.onResolve({filter:/.*/},P=>{if(!/^(?!https?:\/\/).+/.test(P.path))return;let x=P.path;if(x.startsWith("/")&&(x=x.slice(1)),P.importer&&P.importer!==""){let S=P.importer;S.startsWith("virtual-workspace:")&&(S=S.slice(18));const j=P.path.startsWith("./")||P.path.startsWith("../"),A=x.includes("/"),I=S.startsWith("build/");j?x=dt(P.path,S):!A&&!I&&(x=dt(x,S))}return{path:x,namespace:"virtual-workspace"}}),J.onLoad({filter:/.*/},async P=>{if(!/^(?!https?:\/\/).+/.test(P.path))return;const x=P.path,S=await Je.getWorkspace();let j=await S.getResource(x);if(j||(await S.listChildren(!0),j=await S.getResource(x)),!j)throw new Error(`Module not found: ${x}`);return{contents:await j.getContents()}})}};class xn{constructor(){this.initialized=!1,this.workspaceCache=null}async getWorkspace(){return this.workspaceCache||(this.workspaceCache=await Je.getWorkspace()),this.workspaceCache}async init(){this.initialized||(await ft.initialize({wasmURL:Kt}),this.initialized=!0)}async downloadFile(P,x){const S=await this.getWorkspace(),j=""+P,I=await(await fetch(j)).blob();await(await S.getResource(x,{create:!0})).saveContents(I)}async deleteDirectoryIfExists(P){const S=await(await this.getWorkspace()).getResource(P);S&&await S.delete()}async downloadIcons(P,x,S){for(const j of P){const A=j.split("/").pop()||j;x(++S.value,`Downloading icon: ${A}...`),await this.downloadFile(j,`dist/assets/icons/${A}`)}}async cleanBuildDirectories(P,x){P(++x.value,"Cleaning build directories..."),await Promise.all([this.deleteDirectoryIfExists("build"),this.deleteDirectoryIfExists("dist")])}async downloadCoreFiles(P,x,S){const j=[{path:"/lib/gs-lib.js",dest:P,message:"Downloading library files..."},{path:"/lib/gs-lib.css",dest:"dist/app.css",message:"Downloading stylesheet..."},{path:"/pwa/staticwebapp.config.json",dest:"dist/staticwebapp.config.json",message:"Downloading configuration files..."},{path:"/pwa/sw.js",dest:"dist/sw.js",message:"Downloading service worker..."}];for(const A of j)x(++S.value,A.message),await this.downloadFile(A.path,A.dest)}async processServiceWorker(P,x,S){x(++S.value,"Processing service worker...");const I=await(await(await this.getWorkspace()).getResource("dist/sw.js")).getContents();await this.createFile("dist/sw.js",q=>I.replace(/\$PWA_VERSION/g,q.version),P)}async createManifest(P,x,S){x(++S.value,"Creating manifest file..."),await this.createFile("dist/manifest.json",j=>{const A=JSON.parse(JSON.stringify(wn));return A.name=j.title,A.short_name=j.title,A.description=j.title,A.version=j.version,JSON.stringify(A)},P)}async copyWorkspaceAssets(P,x){P(++x.value,"Copying workspace assets...");const j=await(await this.getWorkspace()).getResource("assets");j&&await j.copyTo("dist/assets")}async generateBuildFiles(P,x,S,j,A){j(++A.value,"Generating application code..."),await this.createFile(P,vn,S),j(++A.value,"Generating HTML file..."),await this.createFile(x,yn,S)}async bundleCode(P,x,S,j){S(++j.value,"Bundling and minifying code...");const I=await(await this.getWorkspace()).getResource(x,{create:!0}),q=await ft.build({entryPoints:[P],bundle:!0,outfile:"app.js",format:"esm",minify:!0,plugins:[bn],external:[],packages:"external"});S(++j.value,"Saving bundled output..."),await I.saveContents(q.outputFiles[0].contents)}async cleanupBuild(P,x,S){(P===void 0||P)&&(x(++S.value,"Cleaning up temporary files..."),await this.deleteDirectoryIfExists("build"))}async build(P,x=void 0,S){const j=(ce,ve)=>{S&&(S.currentStep=ce,S.message=ve)};S&&(S.totalSteps=22);const I={value:0};j(++I.value,"Initializing build system..."),await this.init();const q="build/gs-lib.js",ne="build/app.js",re="dist/index.html",se="dist/app.js";await this.cleanBuildDirectories(j,I),await this.downloadCoreFiles(q,j,I);const ee={...P,libPath:q};await this.processServiceWorker(ee,j,I),await this.createManifest(ee,j,I);const ie=["/pwa/assets/icons/24x24.png","/pwa/assets/icons/48x48.png","/pwa/assets/icons/192x192.png","/pwa/assets/icons/512x512.png","/pwa/assets/icons/icon_24.png","/pwa/assets/icons/icon_48.png","/pwa/assets/icons/icon_192.png","/pwa/assets/icons/icon_512.png"];await this.downloadIcons(ie,j,I),await this.copyWorkspaceAssets(j,I),await this.generateBuildFiles(ne,re,ee,j,I),await this.bundleCode(ne,se,j,I),await this.cleanupBuild(x,j,I),j(++I.value,"Finalizing build..."),(await this.getWorkspace()).touch(),S&&(S.message="Build completed successfully!")}async createFile(P,x,S){await(await(await this.getWorkspace()).getResource(P,{create:!0})).saveContents(x(S))}async buildMapFile(P,x){const S=JSON.parse(await P.getContents()),j=await zt(x||".env");j.BUILD_TIME=new Date,ht.runAsync("Building map",async A=>{await qe.build({title:P.getName(),gsMap:S,env:j,version:j.VERSION||"0.0.0"},void 0,A)}).then(()=>{mt("🚀 Map files copied to 'dist' folder in your workspace!")}).catch(A=>{be(`💥 Map could not be built: ${A}`)})}}const qe=new xn;Gt.put("buildService",qe);const je=".geospace",kn=".env",_n=["node_modules",".git","dist","build",".next",".cache",".vscode",".idea","coverage",".nyc_output",".parcel-cache",".turbo",".yarn",".pnp",".svelte-kit",".nuxt",".output","target","out",".temp",".tmp"];function En(J){const P=J.toLowerCase();return _n.some(x=>P===x.toLowerCase())}async function Ge(J,P,x,S={value:0}){const j=[],A=await J.listChildren(!0);for(const I of A){S.value++;const q=I.getWorkspacePath();if(x&&(x.message=`Scanning: ${q}`,x.currentStep=S.value),I instanceof Oe&&I.getName().toLowerCase().endsWith(P))j.push(q);else if(I instanceof Ht&&!En(I.getName())){const ne=await Ge(I,P,x,S);j.push(...ne)}}return j.sort()}function Sn(){let J=document.getElementById("global-dialog-container");return J||(J=document.createElement("div"),J.id="global-dialog-container",document.body.appendChild(J)),J}async function $n(J,P,x,S){return new Promise(j=>{const A=Sn();let I=x||"",q=S||"";const ne=()=>{lt(Be``,A)},re=()=>{ne(),j({geospaceFile:I||null,envFile:q||null})},se=()=>{ne(),j({geospaceFile:null,envFile:null})},ee=Be`
        ...
        `;lt(ee,A)})}const Tn=({commandRegistry:J})=>{J.registerAll({command:{id:"build_map",name:"Build map",description:"Builds a publishable map from a .geospace file",parameters:[{name:"file",description:"Path to the .geospace file relative to workspace root. If not provided, uses the active map editor or selected file.",required:!1},{name:"env",description:"Path to the environment file relative to workspace root (e.g., 'dev.env', 'prod.env'). Defaults to '.env' if not specified.",required:!1},{name:"ask",description:"If true, prompts for missing files (map file or env file) when not provided.",required:!1}]},handler:{execute:async P=>{let x=P.params?.env,S=P.params?.file;const j=P.params?.ask===!0||P.params?.ask==="true",A=await Je.getWorkspace();if(!A){be("No workspace available");return}let I;if(S){const q=await A.getResource(S);if(!q||!(q instanceof Oe)){be(`File not found: ${S}`);return}if(!q.getName().toLowerCase().endsWith(je)){be(`File is not a .geospace file: ${S}`);return}I=q}else{const q=ot.get();if(q&&q.input)q.isDirty()&&(await q.save(),mt("Map saved automatically to run build.")),I=q.input.data;else{const ne=ct.get();ne instanceof Oe&&ne.getName().toLowerCase().endsWith(je)&&(I=ne)}}if(j&&(!I||!x)){let q=[],ne=[];await ht.runAsync("Collecting files",async ee=>{ee.message="Scanning workspace for .geospace files...",q=await Ge(A,je,ee,{value:0}),ee.message="Scanning workspace for .env files...",ee.currentStep=0,ne=await Ge(A,kn,ee,{value:0}),ee.message=`Found ${q.length} .geospace files and ${ne.length} .env files`});const re=I?.getWorkspacePath(),se=await $n(q,ne,re,x||".env");if(se.geospaceFile===null&&se.envFile===null)return;if(se.geospaceFile&&!I){S=se.geospaceFile;const ee=await A.getResource(S);if(!ee||!(ee instanceof Oe)){be(`File not found: ${S}`);return}if(!ee.getName().toLowerCase().endsWith(je)){be(`File is not a .geospace file: ${S}`);return}I=ee}se.envFile!==null&&(x=se.envFile||".env")}else if(!I){be("No map editor active, no .geospace file selected, and no file path provided");return}if(!I){be("No map file available");return}return qe.buildMapFile(I,x)}},contribution:{target:"toolbar:map-editor",icon:"hammer",label:"Build map",disabled:()=>!(ot.get()instanceof Jt)}}),qt.registerContribution("contextmenu:filebrowser",{command:"build_map",icon:"hammer",label:"Build map",params:{ask:!0},disabled:()=>{const P=ct.get();return P instanceof Oe?!P.getName().toLowerCase().endsWith(je):!0}})};export{Tn as default};
