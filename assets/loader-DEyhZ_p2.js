import{C as e,F as t,I as n,L as r,j as i}from"./dist-C1LXzuY9.js";import{m as a,n as o,u as s}from"./fs-access-D-fDaJ8V-BQA-EXV_.js";var c=`#!/usr/bin/env python3
"""
...
def main(input_path, output_path):
    ...
    try:
        ...
        print(f"✗ Error: {e}")
        raise
`;function l(e,t){if(e.startsWith(`/`))return e.slice(1);let n=t.split(`/`).filter(e=>e),r=e.split(`/`).filter(e=>e);(e.startsWith(`./`)||e.startsWith(`../`)||!e.startsWith(`/`))&&n.pop();for(let e of r)e!==`.`&&(e===`..`?n.length>0&&n.pop():n.push(e));return n.join(`/`)}t({command:{id:`gtfs_to_geojson`,name:`Convert GTFS to GeoJSON`,description:`Convert a GTFS zip archive to GeoJSON files`,parameters:[{name:`file`,description:`the GTFS zip file to convert, if not provided, the current selection will be used`,required:!1},{name:`target`,description:`target folder for the output GeoJSON files, defaults to the zip filename + '_geojson'`,required:!1}]},handler:{canExecute:e=>{let t=e.params&&e.params.file;if(!t){let e=i.get();if(!e||!(e instanceof o))return!1;t=e.getWorkspacePath()}return t.toLowerCase().endsWith(`.zip`)},execute:async t=>{let u=t.params&&t.params.file;if(!u){let e=i.get();e instanceof o&&(u=e.getWorkspacePath())}let d=await s.getWorkspace();if(!d){n(`No workspace selected.`);return}await e.runAsync(`Converting GTFS to GeoJSON`,async e=>{try{let i=await d.getResource(u);if(!i||!(i instanceof o)){n(`File not found: `+u);return}let s=i,f=s.getWorkspacePath(),p=t.params&&t.params.target;p||=s.getName().replace(/\.zip$/i,`_geojson`)+`/`;let m=l(p.replace(/\/$/,``),f);e.message=`Preparing conversion...`,e.progress=1,await d.getResource(m,{create:!0});let h=f,g=m;e.message=`Initializing Python environment...`,e.progress=2;let _=a.getProxy().PyEnv;if(!_){n(`Python runtime is not available. Install the system.pythonruntime extension.`);return}let v=new _;await v.init(d),e.message=`Loading required packages...`,e.progress=4,await v.loadPackages([`pandas`,`geopandas`,`shapely`]),e.message=`Loading conversion script...`,e.progress=6;try{await v.execCode(c),e.message=`Executing conversion (this may take a while)...`,e.progress=10,await v.runFunction(`main`,{input_path:h,output_path:g}),e.message=`Conversion complete!`,e.progress=100,r(`GTFS converted successfully to ${g}`)}catch(e){throw n(`Python script failed: `+e),e}}catch(e){throw n(`Failed to convert GTFS: `+e),e}})}},contribution:{target:`contextmenu:filebrowser`,icon:`map-location-dot`,label:`Convert to GeoJSON`,disabled:()=>{let e=i.get();return!e||!(e instanceof o)?!0:!e.getName().toLowerCase().endsWith(`.zip`)}}});