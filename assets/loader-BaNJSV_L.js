import{m as e,n as t,u as n}from"./fs-access-D-fDaJ8V-4fi1R5VX.js";import{L as r,N as i,R as a,w as o,z as s}from"./dist-Bs3vRgSi.js";var c=`#!/usr/bin/env python3
"""
...
def main(input_path, output_path):
    ...
    try:
        ...
        print(f"✗ Error: {e}")
        raise
`;function l(e,t){if(e.startsWith(`/`))return e.slice(1);let n=t.split(`/`).filter(e=>e),r=e.split(`/`).filter(e=>e);(e.startsWith(`./`)||e.startsWith(`../`)||!e.startsWith(`/`))&&n.pop();for(let e of r)e!==`.`&&(e===`..`?n.length>0&&n.pop():n.push(e));return n.join(`/`)}r({command:{id:`gtfs_to_geojson`,name:`Convert GTFS to GeoJSON`,description:`Convert a GTFS zip archive to GeoJSON files`,parameters:[{name:`file`,description:`the GTFS zip file to convert, if not provided, the current selection will be used`,required:!1},{name:`target`,description:`target folder for the output GeoJSON files, defaults to the zip filename + '_geojson'`,required:!1}]},handler:{canExecute:e=>{let n=e.params&&e.params.file;if(!n){let e=i.get();if(!e||!(e instanceof t))return!1;n=e.getWorkspacePath()}return n.toLowerCase().endsWith(`.zip`)},execute:async r=>{let u=r.params&&r.params.file;if(!u){let e=i.get();e instanceof t&&(u=e.getWorkspacePath())}let d=await n.getWorkspace();if(!d){a(`No workspace selected.`);return}await o.runAsync(`Converting GTFS to GeoJSON`,async n=>{try{let i=await d.getResource(u);if(!i||!(i instanceof t)){a(`File not found: `+u);return}let o=i,f=o.getWorkspacePath(),p=r.params&&r.params.target;p||=o.getName().replace(/\.zip$/i,`_geojson`)+`/`;let m=l(p.replace(/\/$/,``),f);n.message=`Preparing conversion...`,n.progress=1,await d.getResource(m,{create:!0});let h=f,g=m;n.message=`Initializing Python environment...`,n.progress=2;let _=e.getProxy().PyEnv;if(!_){a(`Python runtime is not available. Install the system.pythonruntime extension.`);return}let v=new _;await v.init(d),n.message=`Loading required packages...`,n.progress=4,await v.loadPackages([`pandas`,`geopandas`,`shapely`]),n.message=`Loading conversion script...`,n.progress=6;try{await v.execCode(c),n.message=`Executing conversion (this may take a while)...`,n.progress=10,await v.runFunction(`main`,{input_path:h,output_path:g}),n.message=`Conversion complete!`,n.progress=100,s(`GTFS converted successfully to ${g}`)}catch(e){throw a(`Python script failed: `+e),e}}catch(e){throw a(`Failed to convert GTFS: `+e),e}})}},contribution:{target:`contextmenu:filebrowser`,icon:`map-location-dot`,label:`Convert to GeoJSON`,disabled:()=>{let e=i.get();return!e||!(e instanceof t)?!0:!e.getName().toLowerCase().endsWith(`.zip`)}}});