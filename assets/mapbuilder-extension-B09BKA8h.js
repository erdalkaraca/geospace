...
`};let dn={name:"workspace",setup(X){X.onResolve({filter:/.*/},z=>{if(/^(?!https?:\/\/).+/.test(z.path))return{path:z.path,namespace:"virtual-workspace"}}),X.onLoad({filter:/.*/},async z=>{if(!/^(?!https?:\/\/).+/.test(z.path))return;const q=z.path,Y=await Me.getWorkspace();let Q=await Y.getResource(q);return Q||(await Y.listChildren(!0),Q=await Y.getResource(q)),{contents:await Q.getContents()}})}};class hn{constructor(){this.initialized=!1}async init(){this.initialized||(await st.initialize({wasmURL:Wt}),this.initialized=!0)}async downloadFile(z,q){const Y=await Me.getWorkspace(),Q=""+z;await fetch(Q).then(async Z=>Z.blob()).then(async Z=>{await Y.getResource(q,{create:!0}).then(me=>me).then(me=>me.saveContents(Z))})}async build(z,q=void 0){await this.init();const Y="build/gs-lib.js",Q="build/app.js",Z="dist/index.html",me="dist/app.js",ae=await Me.getWorkspace();await ae.getResource("build")&&await(await ae.getResource("build")).delete(),await ae.getResource("dist")&&await(await ae.getResource("dist")).delete(),await this.downloadFile("/lib/gs-lib.js",Y),await this.downloadFile("/lib/gs-lib.css","dist/app.css"),await this.downloadFile("/pwa/staticwebapp.config.json","dist/staticwebapp.config.json"),await this.downloadFile("/pwa/sw.js","dist/sw.js");const we={...z,libPath:Y},ye=await(await ae.getResource("dist/sw.js")).getContents();await this.createFile("dist/sw.js",K=>ye.replace(/\$PWA_VERSION/g,K.version),we),await this.createFile("dist/manifest.json",K=>{const re=JSON.parse(JSON.stringify(cn));return re.name=K.title,re.short_name=K.title,re.description=K.title,re.version=K.version,JSON.stringify(re)},we),await this.downloadFile("/pwa/assets/icons/24x24.png","dist/assets/icons/24x24.png"),await this.downloadFile("/pwa/assets/icons/48x48.png","dist/assets/icons/48x48.png"),await this.downloadFile("/pwa/assets/icons/192x192.png","dist/assets/icons/192x192.png"),await this.downloadFile("/pwa/assets/icons/512x512.png","dist/assets/icons/512x512.png"),await this.downloadFile("/pwa/assets/icons/icon_24.png","dist/assets/icons/icon_24.png"),await this.downloadFile("/pwa/assets/icons/icon_48.png","dist/assets/icons/icon_48.png"),await this.downloadFile("/pwa/assets/icons/icon_192.png","dist/assets/icons/icon_192.png"),await this.downloadFile("/pwa/assets/icons/icon_512.png","dist/assets/icons/icon_512.png");const _e=await ae.getResource("assets");_e&&await _e.copyTo("dist/assets"),await this.createFile(Q,fn,we),await this.createFile(Z,un,we);const Fe=await ae.getResource(me,{create:!0}),Te=await st.build({entryPoints:[Q],bundle:!0,outfile:"app.js",format:"esm",minify:!0,plugins:[dn]});await Fe.saveContents(Te.outputFiles[0].contents),(q===void 0||q)&&await(await ae.getResource("build")).delete(),ae.touch()}async createFile(z,q,Y){await(await(await Me.getWorkspace()).getResource(z,{create:!0})).saveContents(q(Y))}async buildActiveMap(){const z=it.get();z.isDirty()&&(await z.save(),nt("Map saved automatically to run build."));const q=z.input.data,Y=JSON.parse(await q.getContents()),Q=await It(".env","envs/.env","env","envs/env","default.env","envs/default.env","prod.env","envs/prod.env");Q.BUILD_TIME=new Date,Ut.runAsync("Building map",()=>We.build({title:q.getName(),gsMap:Y,env:Q,version:Q.VERSION||"0.0.0"})).then(()=>{nt("🚀 Map files copied to 'dist' folder in your workspace!")}).catch(Z=>{Mt(`💥 Map could not be built: ${Z}`)})}}const We=new hn;Lt.put("buildService",We);const pn=({commandRegistry:X})=>{X.registerAll({command:{id:"build_map",name:"Build map",description:"Builds a publishable map from a .geospace file",parameters:[]},handler:{execute:z=>We.buildActiveMap()},contribution:{target:Ft,icon:"hammer",label:"Build map",disabled:()=>!(it.get()instanceof Nt)}})};export{pn as default};
