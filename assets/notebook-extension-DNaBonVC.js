...
`)||"Unknown error"}:null}renderHeaderActions(e,t){return r`
        ...
        `}renderFooterActions(e){return r`
...
`):void 0,imageData:d}),this.executionCounter++,t.execution_count=this.executionCounter,this.markDirty(!0)}catch(o){this.executingCells.has(e)&&this.cellOutputs.set(e,{type:"error",data:String(o)})}finally{this.executingCells.delete(e),this.requestUpdate()}}}async cancelExecution(e){if(this.pyenv)if(this.pyenv.canInterrupt())this.pyenv.interrupt();else{this.cellOutputs.set(e,{type:"error",data:"Execution cancelled by user (worker terminated - SharedArrayBuffer not available for graceful interrupt)"}),this.executingCells.delete(e),this.pyenv.close(),this.pyenv=void 0,this.pyConnected=!1,this.pyVersion=void 0;try{await this.initPyodide()}catch(t){console.error("Failed to reinitialize Python after cancellation:",t)}this.requestUpdate()}}clearAllOutputs(){this.cellOutputs.clear(),this.executionCounter=0,this.notebook?.cells&&this.notebook.cells.forEach(e=>{e.cell_type==="code"&&(e.execution_count=null,e.outputs=[])}),this.markDirty(!0),this.requestUpdate()}async restartKernel(){if(!(!this.pyenv||this.pyConnecting))try{this.pyConnecting=!0,this.pyenv.close(),this.pyenv=void 0,this.pyConnected=!1,this.pyVersion=void 0,this.requestUpdate(),await this.initPyodide(),this.requestUpdate()}catch(e){console.error("Failed to restart kernel:",e)}finally{this.pyConnecting=!1}}async runAllCells(){if(this.notebook?.cells){this.isRunningAll=!0,this.cancelRunAll=!1,this.requestUpdate();try{for(let e=0;e<this.notebook.cells.length&&!this.cancelRunAll;e++)this.notebook.cells[e].cell_type==="code"&&await this.executeCell(e)}finally{this.isRunningAll=!1,this.cancelRunAll=!1,this.requestUpdate()}}}cancelAllCells(){this.cancelRunAll=!0}toggleMarkdownEdit(e){this.editingMarkdownCells.has(e)?this.editingMarkdownCells.delete(e):this.editingMarkdownCells.add(e),this.requestUpdate()}saveMarkdownEdit(e,t){const i=t.target.value;if(this.notebook&&this.notebook.cells[e]){const a=this.notebook.cells[e],c=this.getCellSource(a);a.source=this.stringToSourceArray(i),c!==i&&this.markDirty(!0)}this.editingMarkdownCells.delete(e),this.requestUpdate()}renderMarkdownCell(e,t){const o=this.getCellSource(e),i=!o||o.trim()==="";if(this.editingMarkdownCells.has(t)){const n=r`
        ...
        `}renderCodeCell(e,t){const o=this.cellOutputs.get(t),i=this.executingCells.has(t);this.cellRefs.has(t)||this.cellRefs.set(t,x());const a=this.cellRefs.get(t),c=i?r`
            <wa-button 
                size="small" 
                appearance="plain"
            ...
            <div class="cell-wrapper">
                <wa-animation 
                    ...
                    <div class="cell code-cell ${i?"executing":""}">
                        ...
                        <div class="cell-input monaco-container" ${_(a)} data-cell-index="${t}"></div>
                        ${o?r`
                            <div class="cell-output ${o.type==="error"?"output-error":""}">
                                ...
                                `:""}
                                ${o.data?r`<pre><code>${o.data}</code></pre>`:""}
                            </div>
                        `:""}
                        ${this.renderFooterActions(t)}
                    </div>
...
`).length,c=19,d=10,n=100,s=Math.max(n,a*c+d);e.style.height=`${s}px`;const l=k.create(e,{value:i,language:"python",theme:this.getMonacoTheme(),minimap:{enabled:!1},scrollBeyondLastLine:!1,lineNumbers:"on",renderLineHighlight:"all",automaticLayout:!0,fontSize:14,tabSize:4,wordWrap:"on"});let m=!1;l.onDidFocusEditorText(()=>{m=!0,this.focusedCellIndex=o}),l.onDidBlurEditorText(()=>{m=!1,this.focusedCellIndex===o&&(this.focusedCellIndex=-1)}),e.addEventListener("wheel",g=>{if(!m){const f=l.getScrollTop(),y=l.getScrollHeight(),w=l.getContentHeight(),v=y>w,C=g.deltaY<0&&f<=0||g.deltaY>0&&f+w>=y;(!v||C)&&g.stopImmediatePropagation()}},{passive:!0,capture:!0}),l.onDidContentSizeChange(()=>{const g=l.getContentHeight();e.style.height=`${Math.max(n,g+d)}px`,l.layout()}),l.onDidChangeModelContent(()=>{const g=l.getValue(),f=this.getCellSource(t);g!==f&&this.markDirty(!0)}),this.monacoEditors.set(o,l)}getMonacoTheme(){const e=document.documentElement;return e.classList.contains("wa-dark")?"vs-dark":e.classList.contains("wa-light")?"vs":"vs-dark"}openPackageManager(){const e=this.notebook?.metadata?.required_packages||[];q.showPackageManager({packages:e,pyenv:this.pyenv,onPackageAdded:t=>{this.notebook&&(this.notebook.metadata||(this.notebook.metadata={}),this.notebook.metadata.required_packages||(this.notebook.metadata.required_packages=[]),this.notebook.metadata.required_packages.includes(t)||(this.notebook.metadata.required_packages.push(t),this.markDirty(!0)))},onPackageRemoved:t=>{if(!this.notebook?.metadata?.required_packages)return;const o=this.notebook.metadata.required_packages.indexOf(t);o>-1&&(this.notebook.metadata.required_packages.splice(o,1),this.markDirty(!0))}})}updated(e){super.updated(e),(e.has("pyConnected")||e.has("pyConnecting")||e.has("pyVersion")||e.has("isRunningAll"))&&this.updateToolbar(),this.notebook?.cells&&this.notebook.cells.forEach((t,o)=>{if(t.cell_type==="code"){const i=this.cellRefs.get(o);i?.value&&!this.monacoEditors.has(o)&&this.initializeMonacoEditor(i.value,t,o)}})}render(){return this.notebook?r`
        ...
        `:r`<div class="loading">Loading notebook...</div>`}};p.styles=$`
        :host {
            display: block;
            width: 100%;
        }
    ...
    `;h([D({attribute:!1})],p.prototype,"input",2);h([u()],p.prototype,"notebook",2);h([u()],p.prototype,"cellOutputs",2);h([u()],p.prototype,"executingCells",2);h([u()],p.prototype,"pyenv",2);h([u()],p.prototype,"pyConnected",2);h([u()],p.prototype,"pyConnecting",2);h([u()],p.prototype,"pyVersion",2);h([u()],p.prototype,"editingMarkdownCells",2);h([u()],p.prototype,"executionCounter",2);h([u()],p.prototype,"isRunningAll",2);h([u()],p.prototype,"highlightedCellIndex",2);p=h([I("k-notebook-editor")],p);const j=({editorRegistry:e,commandRegistry:t})=>{e.registerEditorInputHandler({canHandle:o=>o instanceof E&&o.getName().toLowerCase().endsWith(".ipynb"),handle:async o=>{const i={title:o.getName(),data:o,key:o.getName(),editorId:"notebook-editor",icon:"k jupyter",noOverflow:!0,state:{}};return i.widgetFactory=()=>r`
                <k-notebook-editor .input=${i}></k-notebook-editor>`,i},ranking:100}),t.registerHandler("run_python",{canExecute:o=>{const i=b.get();if(i instanceof p){const a=o.params?.cellIndex;return a!==void 0?a>=0&&a<(i.notebook?.cells.length??0):i.focusedCellIndex>=0}return!1},execute:async o=>{const i=b.get();if(i instanceof p){const a=o.params?.cellIndex??i.focusedCellIndex;a>=0&&await i.executeCell(a)}}})};export{p as KNotebookEditor,j as default};
