import{r as m,d as l,c as p,w as F,t as r,a as y,u as S,b as G}from"./geospace-app-CCGOW35A.js";import"./default-xvQdiKMn.js";import"./chunk.IVRNYRHD-Htfh_CZj.js";import"./preload-helper-WQhrSmCl.js";import"./chunk.2PWIIYRH-COD9mzAh.js";const w=`#!/usr/bin/env python3
"""
...
def main(input_path, output_path):
    ...
    try:
        ...
        print(f"✗ Error: {e}")
        raise
`;function v(e,s){if(e.startsWith("/"))return e.slice(1);const n=s.split("/").filter(o=>o),t=e.split("/").filter(o=>o);(e.startsWith("./")||e.startsWith("../")||!e.startsWith("/"))&&n.pop();for(const o of t)o!=="."&&(o===".."?n.length>0&&n.pop():n.push(o));return n.join("/")}m({command:{id:"gtfs_to_geojson",name:"Convert GTFS to GeoJSON",description:"Convert a GTFS zip archive to GeoJSON files",parameters:[{name:"file",description:"the GTFS zip file to convert, if not provided, the current selection will be used",required:!1},{name:"target",description:"target folder for the output GeoJSON files, defaults to the zip filename + '_geojson'",required:!1}]},handler:{canExecute:e=>{let s=e.params&&e.params.file;if(!s){const n=l.get();if(!n||!(n instanceof p))return!1;s=n.getWorkspacePath()}return s.toLowerCase().endsWith(".zip")},execute:async e=>{let s=e.params&&e.params.file;if(!s){const t=l.get();t instanceof p&&(s=t.getWorkspacePath())}const n=await F.getWorkspace();if(!n){r("No workspace selected.");return}await y.runAsync("Converting GTFS to GeoJSON",async t=>{try{const i=await n.getResource(s);if(!i||!(i instanceof p)){r("File not found: "+s);return}const o=i,_=o.getWorkspacePath();let u=e.params&&e.params.target;u||(u=o.getName().replace(/\.zip$/i,"_geojson")+"/");const d=v(u.replace(/\/$/,""),_);t.message="Preparing conversion...",t.progress=1,await n.getResource(d,{create:!0});const g=_,h=d;t.message="Initializing Python environment...",t.progress=2;const c=S.getProxy().PyEnv;if(!c){r("Python runtime is not available. Install the system.pythonruntime extension.");return}const a=new c;await a.init(n),t.message="Loading required packages...",t.progress=4,await a.loadPackages(["pandas","geopandas","shapely"]),t.message="Loading conversion script...",t.progress=6;try{await a.execCode(w),t.message="Executing conversion (this may take a while)...",t.progress=10,await a.runFunction("main",{input_path:g,output_path:h}),t.message="Conversion complete!",t.progress=100,G(`GTFS converted successfully to ${h}`)}catch(f){throw r("Python script failed: "+f),f}}catch(i){throw r("Failed to convert GTFS: "+i),i}})}},contribution:{target:"contextmenu:filebrowser",icon:"map-location-dot",label:"Convert to GeoJSON",disabled:()=>{const e=l.get();return!e||!(e instanceof p)?!0:!e.getName().toLowerCase().endsWith(".zip")}}});
