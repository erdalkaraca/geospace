import{r as h,d as f,c as a,b as c,P as g,t as m}from"./main-sDPyajcY.js";import{F as p,z as F}from"./index-CzojaPXY.js";const y=`#!/usr/bin/env python3
"""
...
`;h({command:{id:"gtfs_to_geojson",name:"Convert GTFS to GeoJSON",description:"Convert a GTFS zip archive to GeoJSON files",parameters:[{name:"file",description:"the GTFS zip file to convert, if not provided, the current selection will be used",required:!1},{name:"target",description:"target folder for the output GeoJSON files, defaults to the zip filename + '_geojson'",required:!1}]},handler:{canExecute:t=>{let n=t.params&&t.params.file;if(!n){const s=f.get();if(!s||!(s instanceof p))return!1;n=s.getWorkspacePath()}return n.toLowerCase().endsWith(".zip")},execute:async t=>{let n=t.params&&t.params.file;if(!n){const e=f.get();e instanceof p&&(n=e.getWorkspacePath())}const s=await F.getWorkspace();if(!s){a("No workspace selected.");return}await c.runAsync("Converting GTFS to GeoJSON",async e=>{try{const o=await s.getResource(n);if(!o||!(o instanceof p)){a("File not found: "+n);return}const l=o;let i=t.params&&t.params.target;i||(i=l.getName().replace(/\.zip$/i,"_geojson")+"/"),e.message="Preparing conversion...",e.progress=1,await s.getResource(i,{create:!0});const _=l.getWorkspacePath(),d=i.replace(/\/$/,"");e.message="Initializing Python environment...",e.progress=2;const r=new g;await r.init(s),e.message="Loading required packages...",e.progress=4,await r.loadPackages(["pandas","geopandas","shapely"]),e.message="Loading conversion script...",e.progress=6;try{await r.execCode(y),e.message="Executing conversion (this may take a while)...",e.progress=10,await r.runFunction("main",{input_path:_,output_path:d}),e.message="Conversion complete!",e.progress=100,m(`GTFS converted successfully to ${i.replace(/\/$/,"")}`)}catch(u){throw a("Python script failed: "+u),u}}catch(o){throw a("Failed to convert GTFS: "+o),o}})}},contribution:{target:"contextmenu.filebrowser",icon:"map-location-dot",label:"Convert to GeoJSON",disabled:()=>{const t=f.get();return!t||!(t instanceof p)?!0:!t.getName().toLowerCase().endsWith(".zip")}}});
